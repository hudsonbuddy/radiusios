...
#import "PostBeaconTextContent.h"
#import "RadiusRequest.h"

@implementation PostBeaconTextContent

@synthesize connectionText, jsonArray, jsonData;
@synthesize postBeaconTextContentDelegate;

-(void) postBeaconTextContentMethod:(NSNumber *) myNumber :(NSString *)myString {
//    [RadiusRequest setToken:@"74EQux7jsmRvQP0nvxbK8FUsxuDok9Z3loWRH79IW8d7KtRp07"];
//    
//    RadiusRequest *r = [RadiusRequest requestWithParameters:[NSDictionary dictionaryWithObjectsAndKeys:myString,@"text",myNumber,@"beacon",nil] apiMethod:@"post_text_content" httpMethod:@"POST"];
...
//    }];
    ...
    NSLog(@"Posting text content to the new Beacon");
    NSLog(@"%@", myNumber);
    
    int intMyNumber = [myNumber integerValue];
    
    NSString *textURLString = [NSString stringWithFormat:@"http://sc2.pnptsg.com/api/post_text_content"];
    NSLog(@" %@", textURLString);
    ...
    NSString *textDataString = [NSString stringWithFormat:@"token=74EQux7jsmRvQP0nvxbK8FUsxuDok9Z3loWRH79IW8d7KtRp07&beacon=%d&text=%@", intMyNumber, myString];
...
-(void) connection:(NSURLConnection *)conn didReceiveData:(NSData *)data {
    ...
    NSLog(@"this is the text post return content: %@", logString);
    [jsonData appendData:data];
    
    if ([jsonArray count] != 0) {
        
        if([postBeaconTextContentDelegate respondsToSelector:@selector(goToBeacon)]){
            
            [postBeaconTextContentDelegate goToBeacon];
        }
    }
    else {
