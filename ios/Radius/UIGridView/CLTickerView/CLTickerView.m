...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.scrollview == nil) {
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0,
                                                                   ...
                                                                   labelSize.width,
                                                                   self.frame.size.height)];
        label.font = self.marqueeFont; //[UIFont fontWithName:@"QuicksandBold-Regular" size:18.0];
...
{
    [self stopScrolling];
    NSTimer *restartTimer = [NSTimer scheduledTimerWithTimeInterval:3.0
                                                      target:self
                                                    selector:@selector(startScrolling)
                                                    userInfo:nil
                                                     repeats:NO];
}
